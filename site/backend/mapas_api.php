...
function handle_get($pdo, $id, $recurso) {
    try {
        ...
        if ($id) {
            $stmt = $pdo->prepare("SELECT id, identificador, quadra_inicio, quadra_fim, regiao, tipo, obs, grupo_id FROM mapas WHERE id = ?");
            $stmt->execute([$id]);
            echo json_encode($stmt->fetch(PDO::FETCH_ASSOC));
        } else {
            // Atualizado para buscar também o nome do grupo
            $sql = "SELECT m.id, m.identificador, m.quadra_inicio, m.quadra_fim, m.regiao, m.tipo, m.obs, 
    ...
    try {
        switch ($action) {
            case 'create':
                ...
                $sql = "INSERT INTO mapas (identificador, quadra_inicio, quadra_fim, regiao, tipo, obs) VALUES (?, ?, ?, ?, ?, ?)";
                $stmt = $pdo->prepare($sql);
                $stmt->execute([$data['identificador'], $data['quadra_inicio'], $data['quadra_fim'], $data['regiao'], $data['tipo'], $data['obs'] ?? '']);
            ...
            case 'edit_details':
                ...
                $pdo->beginTransaction();
                $sql = "UPDATE mapas SET identificador = ?, quadra_inicio = ?, quadra_fim = ?, regiao = ?, tipo = ?, obs = ? WHERE id = ?";
                $pdo->prepare($sql)->execute([$data['identificador'], $data['quadra_inicio'], $data['quadra_fim'], $data['regiao'], $data['tipo'], $data['obs'] ?? '', $mapa_id]);
            ...
            case 'entregar':
                ...
                if ($mapa_atual && (!empty($mapa_atual['dirigente_id']) || !empty($mapa_atual['grupo_id'])) && ($mapa_atual['dirigente_id'] != ($data['dirigente_id'] ?? null) || $mapa_atual['grupo_id'] != ($data['grupo_id'] ?? null))) {
                    ...
                    $sql_hist = "INSERT INTO historico_mapas 
                                (mapa_id, dirigente_id, grupo_id, data_entrega, data_devolucao, pessoas_faladas_total, dados_quadras) 
                                VALUES (?, ?, ?, ?, ?, ?, ?)";
            ...
            case 'resgatar':
                ...
                $stmt_mapa = $pdo->prepare("SELECT dirigente_id, grupo_id, data_entrega FROM mapas WHERE id = ?");
                ...
                if ($mapa_atual && (!empty($mapa_atual['dirigente_id']) || !empty($mapa_atual['grupo_id']))) {
                    ...
                    $sql_hist = "INSERT INTO historico_mapas (mapa_id, dirigente_id, grupo_id, data_entrega, data_devolucao, pessoas_faladas_total, dados_quadras) VALUES (?, ?, ?, ?, ?, ?, ?)";
                    $pdo->prepare($sql_hist)->execute([$data['mapa_id'], $mapa_atual['dirigente_id'], $mapa_atual['grupo_id'], $mapa_atual['data_entrega'], $data_devolucao_hoje, $total_faladas, $dados_quadras_json]);
                }
            ...
            case 'devolver':
                ...
                if (!empty($mapa_atual['dirigente_id']) || !empty($mapa_atual['grupo_id'])) {
                    $sql_hist = "INSERT INTO historico_mapas (mapa_id, dirigente_id, grupo_id, data_entrega, data_devolucao, pessoas_faladas_total, dados_quadras) VALUES (?, ?, ?, ?, ?, ?, ?)";
                    $pdo->prepare($sql_hist)->execute([$data['mapa_id'], $mapa_atual['dirigente_id'], $mapa_atual['grupo_id'], $mapa_atual['data_entrega'], $data['data_devolucao'], $total_faladas, $dados_quadras_json]);
                }
            ...
            case 'gerar_compartilhamento':
                ...
                $minutos = isset($data['minutos']) ? (int)$data['minutos'] : 90;
