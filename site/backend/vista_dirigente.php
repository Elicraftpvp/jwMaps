                ...
                <div class="col-lg-6 mb-4" id="mapa-card-<?php echo $mapa['id']; ?>">
                    <div class="card shadow-sm h-100">
                        ...
                        <div class="card-body d-flex flex-column">
                            <form class="form-devolver flex-grow-1 d-flex flex-column" data-mapa-id="<?php echo $mapa['id']; ?>">
                                <div class="flex-grow-1">
                                    ...
                                    <div class="list-group list-group-flush mb-3 quadra-list" data-mapa-id="<?php echo $mapa['id']; ?>" style="max-height: 250px; overflow-y: auto;">
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        const saveQuadra = async (quadraId, valor, statusDiv) => {
            ...
            try {
                ...
                if (!response.ok) {
                    const errorResult = await response.json().catch(() => ({ message: 'Falha na comunicação com o servidor.' }));
        ...
        const updateTotal = (quadraList) => {
            ...
            let total = 0;
            quadraList.querySelectorAll('.quadra-input').forEach(input => {
                total += parseInt(input.value, 10) || 0;
            });
            const totalElement = document.getElementById(`total-pessoas-mapa-${mapaId}`);
            if (totalElement) {
        ...
        document.querySelectorAll('.form-devolver').forEach(form => {
            form.addEventListener('submit', async (e) => {
                ...
                if (!dataDevolucao) {
                    showAlert('Por favor, selecione a data de devolução.', 'warning');
                    return;
                }
                ...
                try {
                    ...
                    showAlert(result.message || 'Mapa devolvido com sucesso!', 'success');
                    document.getElementById(`mapa-card-${mapaId}`).remove();

                    if (document.querySelectorAll('.form-devolver').length === 0) {
        ...
        if (pdfModal) {
            pdfModal.addEventListener('show.bs.modal', (event) => {
                ...
                modalTitle.textContent = "Mapa: " + pdfTitle;
                modalBody.innerHTML = ''; 
             ...
             pdfModal.addEventListener('hidden.bs.modal', function () {
                pdfModal.querySelector('#pdf-modal-body').innerHTML = '';
            });
        }
    });
    </script>
