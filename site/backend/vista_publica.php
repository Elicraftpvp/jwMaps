...
try {
    ...
    if (!$dirigente) { http_response_code(403); die("<h1>Acesso Negado</h1>"); }
                    ...
                    <div class="list-group-item quadra-item d-flex justify-content-between align-items-center">
                        ...
                        <div class="d-flex align-items-center">
                            <input type="number" class="form-control form-control-sm text-center quadra-input" style="width: 80px;"
                                ...
                                data-previous-value="<?php echo $quadra['pessoas_faladas']; ?>"
                                data-quadra-id="<?php echo $quadra['id']; ?>" min="0">
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        const saveQuadra = async (quadraId, valor, statusDiv) => {
            ...
            try {
                ...
                return true;
            } catch (error) { statusDiv.innerHTML = '<i class="fas fa-times-circle text-danger"></i>'; return false; }
        };
        document.querySelectorAll('.quadra-input').forEach(input => {
            input.addEventListener('change', async (e) => {
                ...
                if (valorAtual < valorAnterior) { if (!confirm('Diminuindo o número. Continuar?')) { e.target.value = valorAnterior; return; } }
                const success = await saveQuadra(quadraId, valorAtual, statusDiv);
                if (success) { e.target.dataset.previousValue = valorAtual; }
            });
        });
        document.querySelectorAll('.form-devolver').forEach(form => {
            form.addEventListener('submit', async (e) => {
                e.preventDefault();
                // A lógica de devolução é a mesma, pois a API agora faz o trabalho pesado
                const mapaId = e.target.dataset.mapaId;
                const dataDevolucao = document.getElementById(`data_devolucao_${mapaId}`).value;
                if (!confirm('Deseja devolver este mapa? Todos os registros de quadra serão finalizados.')) return;
                try {
                    const response = await fetch(`${API_BASE_URL}/mapas_api.php`, {
                        method: 'PUT', headers: { 'Content-Type': 'application/json' },
