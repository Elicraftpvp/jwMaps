        ...
        try {
            ...
            if (data.recentes.length > 0) {
                data.recentes.forEach(mapa => {
                    ...
                    const item = `<li class="list-group-item"><strong>${mapa.identificador}</strong> com ${mapa.dirigente_nome}<br><small class="text-muted">Entregue em: ${dataEntrega}</small></li>`;
            ...
            if (data.historico.length > 0) {
                data.historico.forEach(h => {
                    ...
                    const item = `<li class="list-group-item"><strong>${h.mapa_identificador}</strong> por ${h.dirigente_nome}<br><small class="text-muted">Devolvido em: ${dataDevolucao} (${h.pessoas_faladas_total} pessoas)</small></li>`;
