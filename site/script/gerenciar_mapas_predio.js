...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const carregarMapas = async (manterVisual = false, fetchNovo = true) => {
        ...
        try {
            ...
            if (fetchNovo) {
                const response = await fetch(`${API_BASE_URL}/mapas_predio_api.php`);
    ...
    const carregarHistoricoMapa = async (mapaId, identificador) => {
        ...
        try {
            const response = await fetch(`${API_BASE_URL}/mapas_predio_api.php?recurso=history&id=${mapaId}`);
            if (!response.ok) throw new Error(await handleApiError(response));
    ...
    document.getElementById('salvar-mapa-btn').addEventListener('click', async () => {
        ...
        const url = `${API_BASE_URL}/mapas_predio_api.php`;
        if (editMode) {
    ...
    const carregarDirigentesNoModal = async () => {
        try {
            const response = await fetch(`${API_BASE_URL}/mapas_predio_api.php?recurso=dirigentes`);
            if (!response.ok) throw new Error(await handleApiError(response));
    ...
    document.getElementById('confirmar-entrega-btn').addEventListener('click', async () => {
        ...
        try {
            const response = await fetch(`${API_BASE_URL}/mapas_predio_api.php`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(data) });
            if (!response.ok) throw new Error(await handleApiError(response));
            entregarModal.hide();
            carregarMapas(true);
